...
class ExternalShuffleService(sparkConf: SparkConf, securityManager: SecurityManager)
  ...
  private val transportContext: TransportContext = TransportContext.ContextFactory.createTransportContext(transportConf, blockHandler)
  ...
  def start() {
    ...
    logInfo("Starting shuffle service on port $port with useSasl = $useSasl")
    if (useSasl) {
      if (transportContext.isInstanceOf[NettyTransportContext]) {
        val bootstraps = Seq(new SaslServerBootstrap(transportConf, securityManager))
        server = transportContext.asInstanceOf[NettyTransportContext].createServer(port, bootstraps)
      } else {
        logError("Cannot start Shuffle Service, Only Netty can be used with sasl")
      }
    } else {
